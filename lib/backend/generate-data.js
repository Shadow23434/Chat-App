...
// Tạo messages
const generateMessages = (userIds, chatIds) => {
    return Array.from({ length: DATA.messages }, () => ({
        _id: { $oid: new ObjectId().toString() },
        chatId: { $oid: sample(chatIds) },
        senderId: { $oid: sample(userIds) },
        content: faker.lorem.sentence(),
...
const generateData = () => {
    ...
    const chatIds = chats.map(chat => chat._id.$oid);
    fs.writeFileSync("chats.json", JSON.stringify(chats, null, 2));

    // 6. Generate messages
    const messages = generateMessages(userIds, chatIds);
